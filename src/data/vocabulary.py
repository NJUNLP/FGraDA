...
class Vocabulary(object):
    ...
    def build_from_file(type: str, dict_path: str, max_n_words: int, **kwargs) -> 'Vocabulary':
        ...
        if type == "word":
            ...
            return BPEVocabulary(dictionary=dictionary, 
                                 max_n_words=max_n_words, 
                                 codes=kwargs['codes'],
                                 apply_mod=kwargs['apply_mod'])
        else:
...
class BPEVocabulary(Vocabulary):
    ...
    def __init__(self, dictionary: dict, codes: str, apply_mod: bool, max_n_words=-1):
        super().__init__(dictionary=dictionary, max_n_words=max_n_words)
        self.bpe = Bpe(codes=codes, apply_mod=apply_mod)
